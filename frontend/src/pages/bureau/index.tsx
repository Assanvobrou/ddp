                ...
                <tbody>
                  {filtered.map(p => (
                    <tr key={p.id} onClick={() => navigate(`/bureau/patients/${p.id}`)}
                      ...
                      <td className="px-5 py-3.5 font-bold text-ink">{p.prenom} {p.nom}</td>
                      <td className="px-5 py-3.5 text-ink-muted">{p.sexe === 'F' ? 'Féminin' : p.sexe === 'M' ? 'Masculin' : 'Autre'}</td>
                      <td className="px-5 py-3.5 text-ink-muted">{p.age ? `${p.age} ans` : p.date_naissance || '—'}</td>
                      <td className="px-5 py-3.5 text-ink-muted">{p.telephone || '—'}</td>
                      <td className="px-5 py-3.5">
                        {p.a_assurance
