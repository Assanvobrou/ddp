                ...
                <tbody>
                  {(fiches as any[]).map(f => (
                    <tr key={f.id} className="border-b border-surface-50 hover:bg-surface-50">
                      <td className="px-3 md:px-5 py-3 font-bold text-ink">{f.patient_nom || '—'}</td>
                      <td className="px-3 md:px-5 py-3.5"><code className="bg-primary-50 text-primary-700 px-1.5 py-0.5 rounded text-xs font-bold">{f.patient_code || '—'}</code></td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{f.prestation_nom}</td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{f.service_nom || '—'}</td>
                      <td className="px-3 md:px-5 py-3 font-bold">{fmt(f.montant_patient)}</td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{fmt(f.montant_assurance)}</td>
                      <td className="px-3 md:px-5 py-3 text-xs text-ink-faint">{format(new Date(f.date_creation), 'dd/MM HH:mm')}</td>
...
export function Versements() {
  ...
  return (
    <AppLayout>
      ...
      <div className="p-4 md:p-6 space-y-4 md:space-y-4 md:space-y-5">
        <Card padding={false}>
                ...
                <tbody>
                  {(sessions as any[]).map(s => (
                    <tr key={s.id} className="border-b border-surface-50 hover:bg-surface-50">
                      <td className="px-3 md:px-5 py-3 font-semibold">{s.date_session}</td>
                      <td className="px-3 md:px-5 py-3.5">{s.ouverte_par_nom}</td>
                      <td className="px-3 md:px-5 py-3 font-bold">{fmt(s.montant_systeme)}</td>
                      <td className="px-3 md:px-5 py-3.5">{s.montant_compte ? fmt(s.montant_compte) : '—'}</td>
                      <td className="px-3 md:px-5 py-3.5">{s.montant_recu_comptable ? fmt(s.montant_recu_comptable) : '—'}</td>
                      ...
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{s.valide_par_nom || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export function Rapports() {
  ...
  const caissiers = (users as any[]).filter((u: any) =>
    ['caissiere', 'comptable'].includes(u.role)
  )
        ...
        <Card>
          <div className="flex items-end gap-3 flex-wrap">
            ...
            {isDirectrice && caissiers.length > 0 && (
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-ink-muted">Caissière</label>
                <select value={caissiereFiltre} onChange={e => setCaissiereFiltre(e.target.value)}
                  className="h-11 px-3 bg-surface-50 border border-surface-200 rounded-xl text-sm outline-none focus:border-primary-600">
                    ...
                    <tbody>
                      {data.sessions.map((s: any) => (
                        <tr key={s.id} className="border-b border-surface-50 hover:bg-surface-50">
                          <td className="px-3 md:px-5 py-3 font-semibold">{s.date_session}</td>
                          <td className="px-3 md:px-5 py-3.5">{s.ouverte_par_nom}</td>
                          <td className="px-3 md:px-5 py-3 font-bold text-primary-700">{fmt(Number(s.montant_systeme))}</td>
                          <td className="px-3 md:px-5 py-3.5">{s.montant_compte ? fmt(Number(s.montant_compte)) : '—'}</td>
                          <td className="px-3 md:px-5 py-3.5">
