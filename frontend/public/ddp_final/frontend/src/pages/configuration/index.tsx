                ...
                <tbody>
                  {(services as any[]).map((s: any) => (
                    <tr key={s.id} className="border-b border-surface-50 hover:bg-surface-50">
                      <td className="px-3 md:px-5 py-3 font-semibold text-ink">{s.nom}</td>
                      <td className="px-3 md:px-5 py-3.5"><code className="bg-surface-100 px-2 py-0.5 rounded text-xs font-bold">{s.code}</code></td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted text-xs">{s.description || '—'}</td>
                      <td className="px-3 md:px-5 py-3 text-ink-faint">{s.ordre}</td>
                ...
                <tbody>
                  {(prestations as Prestation[]).map(p => (
                    <tr key={p.id} className="border-b border-surface-50 hover:bg-surface-50">
                      <td className="px-3 md:px-5 py-3 font-semibold text-ink">{p.nom}</td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{(p as any).service_nom || '—'}</td>
                      <td className="px-3 md:px-5 py-3 font-bold">{Number(p.prix).toLocaleString('fr-FR')} FCFA</td>
                      <td className="px-3 md:px-5 py-3.5">
                        <Badge variant={p.prise_en_charge_assurance ? 'success' : 'neutral'}>
                          {p.prise_en_charge_assurance ? 'Oui' : 'Non'}
                        </Badge>
                      </td>
                      <td className="px-3 md:px-5 py-3 text-ink-muted">{p.prise_en_charge_assurance ? 'Selon assurance' : '—'}</td>
                      <td className="px-3 md:px-5 py-3 text-ink-faint">{p.ordre}</td>
                      <td className="px-3 md:px-5 py-3.5">
                        <div className="flex items-center gap-1">
                ...
                <tbody>
                  {(assurances as Assurance[]).map(a => (
                    <tr key={a.id} className="border-b border-surface-50 hover:bg-surface-50">
                      <td className="px-3 md:px-5 py-3 font-semibold text-ink">{a.nom}</td>
                      <td className="px-3 md:px-5 py-3.5"><code className="bg-surface-100 px-2 py-0.5 rounded text-xs font-bold">{a.code}</code></td>
                      <td className="px-3 md:px-5 py-3 font-bold text-primary-600">{a.taux_defaut}%</td>
                      <td className="px-3 md:px-5 py-3.5">
