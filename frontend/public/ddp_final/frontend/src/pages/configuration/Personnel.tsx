...
function ModalCreation({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-[95vw] md:max-w-lg max-h-[90vh] overflow-y-auto p-6 animate-fade-up">
                ...
                <tbody>
                  {(users as any[]).map(u => (
                    <tr key={u.id} className="border-b border-surface-50 hover:bg-surface-50">
                      ...
                      <td className="px-3 md:px-5 py-3.5"><code className="text-xs bg-surface-100 text-primary-700 px-2 py-0.5 rounded font-bold">{u.matricule||'—'}</code></td>
                      <td className="px-3 md:px-5 py-3.5">
                      ...
                      <td className="px-3 md:px-5 py-3.5"><Badge variant={u.is_active?'success':'error'}>{u.is_active?'Actif':'Inactif'}</Badge></td>
                      <td className="px-3 md:px-5 py-3 text-xs text-ink-faint">{format(new Date(u.date_creation),'dd/MM/yyyy',{locale:fr})}</td>
                      <td className="px-3 md:px-5 py-3.5">
